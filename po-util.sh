...
echo
  ...
  cd "$FIRMWARE_PI" || exit
    ...
    curl -s https://po-util-tracker.herokuapp.com/install/$USER/$HOSTNAME@$(uname -s)/$(curl ipecho.net/plain) > /dev/null
...
sudo mv po.1 /usr/local/share/man/man1/
sudo mandb &> /dev/null
curl -s https://po-util-tracker.herokuapp.com/update/$USER/$HOSTNAME@$(uname -s)/$(curl ipecho.net/plain) > /dev/null
echo
exit
fi
